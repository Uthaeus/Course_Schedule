    ...
    return (
      <li key={course.title} className={`course ${course.open ? 'course__selected' : ''}` }>
        <div className="course__info">
          ...
          <a className={`course__add action ${course.enrolled ? 'course__remove' : 'course__add'}`} onClick={() => course.enrolled ? this.props.removeCourse(course) : this.props.addCourse(course)}></a>
